...
function getOutputChannel(): vscode.OutputChannel {
  if (!outputChannel) {
    outputChannel = vscode.window.createOutputChannel("better-ctx");
  }
...
async function runInOutputChannel(
  ...
  channel.show(true);
  channel.appendLine(`\n━━━ ${title} ━━━`);
  channel.appendLine(`> better-ctx ${args.join(" ")}\n`);
  ...
  try {
    ...
    vscode.window.showErrorMessage(`better-ctx: ${message}`);
  }
}
...
export async function cmdDashboard(): Promise<void> {
  ...
  try {
    ...
    channel.appendLine(`Error: ${message}`);
    channel.appendLine(
      "Tip: Run 'better-ctx dashboard' in terminal for the web dashboard."
    );
  }
}
...
export async function showWelcome(): Promise<void> {
  ...
  if (version) {
    const channel = getOutputChannel();
    channel.appendLine(
      `better-ctx v${version} activated — 42 MCP tools, 10 read modes, 90+ compression patterns`
    );
  }
}
