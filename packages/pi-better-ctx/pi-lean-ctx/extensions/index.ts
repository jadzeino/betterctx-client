...
function resolveBinary(): string {
  const envBin = process.env.BETTER_CTX_BIN;
  ...
  const candidates = isWin
    ? [
        resolve(home, ".cargo", "bin", "better-ctx.exe"),
        resolve(home, "AppData", "Local", "better-ctx", "better-ctx.exe"),
      ]
    : [
        resolve(home, ".cargo", "bin", "better-ctx"),
        resolve(home, ".local", "bin", "better-ctx"),
...
function parseLeanCtxOutput(text: string) {
  ...
  for (const line of lines) {
    ...
    const shellMatch = trimmed.match(/^\[better-ctx:\s*(\d+)\s*→\s*(\d+)\s*tok,\s*-?(\d+)%\]$/);
    if (shellMatch) {
...
async function execLeanCtx(pi: ExtensionAPI, args: string[]) {
  ...
  const result = await pi.exec(bin, args, { env: { BETTER_CTX_COMPRESS: "1" } });
  if (result.code !== 0) {
    const msg = (result.stderr || result.stdout || `better-ctx failed: ${args.join(" ")}`).trim();
    throw new Error(msg);
  }
...
export default function (pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...baseBashTool,
    description:
      "Execute a bash command through better-ctx compression for 60-90% smaller output.",
    promptSnippet: "Run shell commands through better-ctx compression.",
    promptGuidelines: [
      "Use bash normally — commands are automatically routed through better-ctx.",
      "better-ctx compresses verbose CLI output (git, cargo, npm, docker, kubectl, etc.) automatically.",
    ],
  ...
  pi.registerTool({
    ...
    label: "Read",
    description:
      "Read file contents through better-ctx with automatic mode selection (full/map/signatures) based on file type and size.",
    promptSnippet: "Read files through better-ctx compression with smart mode selection.",
    promptGuidelines: [
      "Use read normally — better-ctx automatically selects the optimal compression mode.",
    ...
    async execute(_toolCallId, params, signal, onUpdate, ctx) {
      ...
      if (params.offset !== undefined || params.limit !== undefined) {
        ...
        try {
          ...
          return {
            content: [{ type: "text", text: decorated.text }],
            details: { path: absolutePath, lines: originalSlice.lines, source: "better-ctx", mode: `lines:${startLine}-${endLine}`, compression: decorated.stats },
          };
        } catch {
      ...
      return {
        content: [{ type: "text", text: decorated.text }],
        details: { path: absolutePath, source: "better-ctx", mode, compression: decorated.stats },
      };
    },
  });
  ...
  pi.registerTool({
    name: "ls",
    label: "ls",
    description: "List directory contents through better-ctx compression.",
    promptSnippet: "List directory contents with token-optimized output.",
    promptGuidelines: ["Use ls normally — output is automatically compressed by better-ctx."],
    parameters: lsSchema,
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      return {
        content: [{ type: "text", text: decorated.text }],
        details: { path: absolutePath, source: "better-ctx", truncated: decorated.truncated, compression: decorated.stats },
      };
    },
  });
  ...
  pi.registerTool({
    name: "find",
    label: "find",
    description: "Find files by glob pattern through better-ctx compression.",
    promptSnippet: "Find files with compressed output.",
    promptGuidelines: ["Use find normally — output respects .gitignore and is compressed by better-ctx."],
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      return {
        content: [{ type: "text", text: decorated.text }],
        details: { path: absolutePath, pattern: params.pattern, source: "better-ctx", truncated: decorated.truncated, compression: decorated.stats },
      };
    },
  });
  ...
  pi.registerTool({
    name: "grep",
    label: "grep",
    description: "Search file contents through ripgrep + better-ctx compression.",
    promptSnippet: "Search code with compressed, grouped results.",
    promptGuidelines: ["Use grep normally — results are compressed and grouped by better-ctx."],
    ...
    async execute(_toolCallId, params, _signal, _onUpdate, ctx) {
      ...
      return {
        content: [{ type: "text", text: decorated.text }],
        details: { path: absolutePath, pattern: params.pattern, source: "better-ctx", compression: decorated.stats },
      };
    },
  });
