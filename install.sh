#!/usr/bin/env bash
# install.sh — Install better-ctx (download pre-built binary or build from source)
#
# Usage:
...
REPO="jadzeino/betterctx-client"
INSTALL_DIR="${BETTER_CTX_INSTALL_DIR:-$HOME/.local/bin}"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]:-install.sh}")" 2>/dev/null && pwd || pwd)"
RUST_DIR="$SCRIPT_DIR/rust"

echo "better-ctx installer"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
...
finish() {
  ...
  echo ""
  echo "Done! Verify with: better-ctx --version"
}
...
install_download() {
  ...
  local asset_url="https://github.com/${REPO}/releases/download/${latest}/better-ctx-${target}.tar.gz"
  ...
  if ! curl -fsSL "$asset_url" -o "$tmpdir/better-ctx.tar.gz"; then
  ...
  if curl -fsSL "$sums_url" -o "$tmpdir/SHA256SUMS" 2>/dev/null; then
    local expected
    expected="$(grep "better-ctx-${target}.tar.gz" "$tmpdir/SHA256SUMS" | cut -d' ' -f1)"
    if [[ -n "$expected" ]]; then
      verify_checksum "$tmpdir/better-ctx.tar.gz" "$expected"
    fi
  else
    echo "  Warning: checksums not available, skipping verification"
  fi

  tar -xzf "$tmpdir/better-ctx.tar.gz" -C "$tmpdir"

  mkdir -p "$INSTALL_DIR"
  install -m755 "$tmpdir/better-ctx" "$INSTALL_DIR/better-ctx"

  if [[ "$(uname -s)" == "Darwin" ]]; then
    xattr -cr "$INSTALL_DIR/better-ctx" 2>/dev/null || true
    codesign --force --sign - "$INSTALL_DIR/better-ctx" 2>/dev/null || true
  fi

  echo "  Installed: $INSTALL_DIR/better-ctx"
...
install_from_source() {
  ...
  echo ""
  echo "Building better-ctx (release)..."

  if [[ -d "$RUST_DIR" ]]; then
    (cd "$RUST_DIR" && cargo build --release)
    local binary="$RUST_DIR/target/release/better-ctx"
  else
  ...
  mkdir -p "$INSTALL_DIR"
  ln -sf "$binary" "$INSTALL_DIR/better-ctx"
  echo "  Linked: $INSTALL_DIR/better-ctx -> $binary"
...
case "${1:-}" in
  ...
  --help|-h)
    ...
    echo ""
    echo "Environment:"
    echo "  BETTER_CTX_INSTALL_DIR  Custom install directory (default: ~/.local/bin)"
    ;;
  *)             install_from_source ;;
esac
