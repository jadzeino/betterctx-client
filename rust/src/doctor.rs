//! Environment diagnostics for better-ctx installation and integration.
...
fn path_in_path_env() -> bool {
    if let Ok(path) = std::env::var("PATH") {
        for dir in std::env::split_paths(&path) {
            if dir.join("better-ctx").is_file() {
                return true;
            }
            if cfg!(windows)
                && (dir.join("better-ctx.exe").is_file() || dir.join("better-ctx.cmd").is_file())
            {
                return true;
            }
        }
...
fn resolve_better_ctx_binary() -> Option<PathBuf> {
    #[cfg(unix)]
    {
        let output = std::process::Command::new("/bin/sh")
            .arg("-c")
            .arg("command -v better-ctx")
...
fn better_ctx_version_from_path() -> Outcome {
    let resolved = resolve_better_ctx_binary();
    let bin = resolved
        .clone()
        .unwrap_or_else(|| std::env::current_exe().unwrap_or_else(|_| "better-ctx".into()));
    ...
    match try_run(&bin) {
        Ok(text) => Outcome {
            ok: true,
            line: format!("{BOLD}better-ctx version{RST}  {WHITE}{text}{RST}"),
        },
        Err(_first_err) => {
            #[cfg(windows)]
            {
                    ...
                    if candidate.is_file() {
                        if let Ok(text) = try_run(candidate) {
                            return Outcome {
                                ok: true,
                                line: format!(
                                    "{BOLD}better-ctx version{RST}  {WHITE}{text}{RST}  {DIM}(via {}){RST}",
                                    candidate.display()
                                ),
                            };
                        }
            ...
            if let Ok(ref exe) = current_exe_result {
                if exe != &bin {
                    if let Ok(text) = try_run(exe) {
                        return Outcome {
                            ok: true,
                            line: format!("{BOLD}better-ctx version{RST}  {WHITE}{text}{RST}  {DIM}(this binary){RST}"),
                        };
                    }
                }
            }
            ...
            Outcome {
                ok: false,
                line: format!(
                    "{BOLD}better-ctx version{RST}  {RED}failed to run `better-ctx --version`: {_first_err}{RST}  {DIM}(resolved: {}){RST}",
                    bin.display()
                ),
            }
        }
...
fn rc_contains_better_ctx(path: &PathBuf) -> bool {
    match std::fs::read_to_string(path) {
        Ok(s) => s.contains("better-ctx"),
        Err(_) => false,
    }
}
...
fn shell_aliases_outcome() -> Outcome {
    ...
    if parts.is_empty() {
        let hint = if cfg!(windows) {
            "no \"better-ctx\" in PowerShell profile, ~/.zshrc or ~/.bashrc"
        } else {
            "no \"better-ctx\" in ~/.zshrc, ~/.bashrc, or ~/.config/fish/config.fish"
        };
        Outcome {
            ok: false,
        ...
        Outcome {
            ok: true,
            line: format!(
                "{BOLD}Shell aliases{RST}  {GREEN}better-ctx referenced in {}{RST}",
                parts.join(", ")
            ),
        }
    }
...
fn mcp_config_outcome() -> Outcome {
    ...
    for loc in &locations {
        match std::fs::read_to_string(&loc.path) {
            Ok(content) if content.contains("better-ctx") => {
    ...
    if !found.is_empty() {
        Outcome {
            ok: true,
            line: format!(
                "{BOLD}MCP config{RST}  {GREEN}better-ctx found in: {}{RST}",
                found.join(", ")
            ),
        }
        ...
        Outcome {
            ok: false,
            line: format!(
                "{BOLD}MCP config{RST}  {YELLOW}config exists for {} but does not reference better-ctx{RST}  {DIM}(run: better-ctx setup){RST}",
                exists_no_ref.join(", ")
            ),
        }
    } else {
        Outcome {
            ok: false,
            line: format!(
                "{BOLD}MCP config{RST}  {YELLOW}no MCP config found{RST}  {DIM}(run: better-ctx setup){RST}"
            ),
        }
    }
}
        ...
        Ok(output) if output.status.success() => {
            ...
            if has_plugin && has_mcp {
                ...
                Some(Outcome {
                    ok: false,
                    line: format!(
                        "{BOLD}Pi Coding Agent{RST}  {YELLOW}{version}, but pi-better-ctx not installed{RST}  {DIM}(run: pi install npm:pi-better-ctx){RST}"
                    ),
                })
            }
        }
...
fn tool_adoption_outcome() -> Outcome {
    ...
    if total_mode_calls == 0 && shell_commands == 0 {
        return Outcome {
            ok: true,
            line: format!(
                "{BOLD}Tool adoption{RST}  {YELLOW}no usage data yet{RST}  {DIM}(use better-ctx tools to see adoption stats){RST}"
            ),
        };
    }
        ...
        return Outcome {
            ...
            line: format!(
                "{BOLD}Tool adoption{RST}  {RED}shell hook active but MCP data tools not used{RST}\n\
                 ...
                 {DIM}     Fix: run {BOLD}better-ctx init{RST}{DIM} to update rules, or check for conflicting{RST}\n\
...
pub fn run() {
    ...
    println!("{BOLD}{WHITE}better-ctx doctor{RST}  {DIM}diagnostics{RST}\n");

    // 1) Binary on PATH
    let path_bin = resolve_better_ctx_binary();
    ...
    let bin_line = if let Some(p) = path_bin {
        format!("{BOLD}better-ctx in PATH{RST}  {WHITE}{}{RST}", p.display())
    } else if also_in_path_dirs {
        format!(
            "{BOLD}better-ctx in PATH{RST}  {YELLOW}found via PATH walk (not resolved by `command -v`){RST}"
        )
    } else {
        format!("{BOLD}better-ctx in PATH{RST}  {RED}not found{RST}")
    };
    print_check(&Outcome {
        ok: bin_ok,
        line: bin_line,
    ...
    let ver = if bin_ok {
        better_ctx_version_from_path()
    } else {
        Outcome {
            ok: false,
            line: format!("{BOLD}better-ctx version{RST}  {RED}skipped (binary not in PATH){RST}"),
        }
    };
    if ver.ok {
        passed += 1;
    ...
    let dir_outcome = match &lean_dir {
        Some(p) if p.is_dir() => {
            ...
            Outcome {
                ok: true,
                line: format!(
                    "{BOLD}~/.better-ctx/{RST}  {GREEN}exists{RST}  {DIM}{}{RST}",
                    p.display()
                ),
            }
        }
        Some(p) => Outcome {
            ok: false,
            line: format!(
                "{BOLD}~/.better-ctx/{RST}  {RED}missing or not a directory{RST}  {DIM}{}{RST}",
                p.display()
            ),
        },
        None => Outcome {
            ok: false,
            line: format!("{BOLD}~/.better-ctx/{RST}  {RED}could not resolve home directory{RST}"),
        },
    };
