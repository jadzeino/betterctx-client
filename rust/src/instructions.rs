...
pub fn build_instructions_with_client(crp_mode: CrpMode, client_name: &str) -> String {
    ...
    let mut base = format!("\
CRITICAL: ALWAYS use better-ctx MCP tools instead of native equivalents for token savings.\n\
\n\
better-ctx MCP — MANDATORY tool mapping:\n\
...
• Edit/StrReplace -> use native (better-ctx only replaces READ, not WRITE)\n\
• Write, Delete, Glob -> use normally\n\
\n\
COMPATIBILITY: better-ctx replaces READ operations only. Your native Edit/Write/StrReplace tools \
remain unchanged. If your instructions say 'use Edit or Write tools only', that is compatible — \
better-ctx only changes how you READ files (ctx_read), not how you EDIT them.\n\
\n\
...
fn should_use_unified(client_name: &str) -> bool {
    if std::env::var("BETTER_CTX_FULL_TOOLS").is_ok() {
        return false;
    }
    if std::env::var("BETTER_CTX_UNIFIED").is_ok() {
        return true;
    }
    let _ = client_name;
    false
