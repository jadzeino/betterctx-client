use better_ctx::core::events::{emit, EventKind};
use better_ctx::core::gotcha_tracker::{
    Gotcha, GotchaCategory, GotchaSeverity, GotchaSource, GotchaStats, GotchaStore,
};
use better_ctx::core::knowledge::ProjectKnowledge;
...
fn seed_gotchas(project_root: &str) {
    ...
    let gotchas_data = vec![
        ...
        (GotchaCategory::Build, GotchaSeverity::Critical, "error: failed to resolve: use of unresolved module `tui`", "Add `pub mod tui;` to lib.rs and `use better_ctx::tui;` in main.rs.", 2, 0.95, 1),
...
fn seed_feedback() {
    use better_ctx::core::feedback::{CompressionOutcome, FeedbackStore};
