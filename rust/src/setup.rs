...
pub fn run_setup() {
    ...
    if total_ok == 0 && errors.is_empty() {
        terminal_ui::print_status_warn(
            "No AI tools detected. Install one and re-run: better-ctx setup",
        );
    }
    ...
    let lean_dir = home.join(".better-ctx");
    if !lean_dir.exists() {
        let _ = std::fs::create_dir_all(&lean_dir);
        terminal_ui::print_status_new("Created ~/.better-ctx/");
    } else {
        terminal_ui::print_status_ok("~/.better-ctx/ ready");
    }
    crate::doctor::run_compact();

    // Step 5: Data sharing
    terminal_ui::print_step_header(5, 5, "Help Improve better-ctx");
    println!("  Share anonymous compression stats to make better-ctx better.");
    ...
    if contribute {
        let config_dir = home.join(".better-ctx");
        ...
        terminal_ui::print_status_skip("Skipped — enable later with: better-ctx config");
    }
    ...
    println!();
    println!(
        "  {dim}After restart, better-ctx will automatically optimize every AI interaction.{rst}"
    );
    println!("  {dim}Verify with:{rst} {bold}better-ctx gain{rst}");
