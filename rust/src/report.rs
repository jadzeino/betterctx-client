//! `better-ctx report-issue` — collects diagnostics and creates a GitHub issue.

use std::path::PathBuf;

const VERSION: &str = env!("CARGO_PKG_VERSION");
const REPO: &str = "jadzeino/betterctx-client";
...
pub fn run(args: &[String]) {
    ...
    println!("{BOLD}better-ctx report-issue{RST}\n");
    ...
    if dry_run {
        ...
        if let Some(dir) = better_ctx_dir() {
...
fn section_mcp_status() -> String {
    ...
    let binary_ok = which_better_ctx().is_some();
    lines.push(format!(
        "- Binary on PATH: {}",
...
fn try_gh_cli(title: &str, body: &str) -> bool {
    ...
    let tmp = std::env::temp_dir().join("better-ctx-report.md");
...
fn try_ureq_api(title: &str, body: &str) {
    ...
    match ureq::post(&url)
        ...
        .header("User-Agent", &format!("better-ctx/{VERSION}"))
...
fn better_ctx_dir() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".better-ctx"))
}

fn which_better_ctx() -> Option<PathBuf> {
