...
pub fn analyze_history(history: &[String], limit: usize) -> DiscoverResult {
    ...
    for cmd in history {
        ...
        if trimmed.starts_with("better-ctx ") {
            already_optimized += 1;
            continue;
        }
...
pub fn discover_from_history(history: &[String], limit: usize) -> String {
    ...
    lines.push(format!(
        "{total_missed} commands could benefit from better-ctx:"
    ));
    lines.push(String::new());
    ...
    lines.push(String::new());
    lines.push("Fix: run 'better-ctx init --global' to auto-compress all commands.".to_string());
    lines.push("Or:  run 'better-ctx init --agent <tool>' for AI tool hooks.".to_string());
...
pub fn format_cli_output(result: &DiscoverResult) -> String {
    if result.missed_commands.is_empty() {
        return format!(
            "All compressible commands are already using better-ctx!\n\
             ({} commands analyzed, {} via better-ctx)",
            result.total_commands, result.already_optimized
        );
    }
    ...
    lines.push(format!(
        "Found {total_missed} compressible commands not using better-ctx:\n"
    ));
    lines.push(format!(
        "  {:<14} {:>5}  {:>10}  {:<30} {}",
    ...
    lines.push(format!(
        "Already using better-ctx: {} commands",
