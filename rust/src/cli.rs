...
pub fn cmd_diff(args: &[String]) {
    if args.len() < 2 {
        eprintln!("Usage: better-ctx diff <file1> <file2>");
        std::process::exit(1);
    }
...
pub fn cmd_grep(args: &[String]) {
    if args.is_empty() {
        eprintln!("Usage: better-ctx grep <pattern> [path]");
        std::process::exit(1);
    }
...
pub fn cmd_find(args: &[String]) {
    if args.is_empty() {
        eprintln!("Usage: better-ctx find <pattern> [path]");
        std::process::exit(1);
    }
...
pub fn cmd_session() {
    ...
    println!("better-ctx session statistics\n");
    println!(
    ...
    println!("\nRun 'better-ctx discover' for details on missed commands.");
}
...
pub fn cmd_sessions(args: &[String]) {
    ...
    match action {
        ...
        _ => {
            eprintln!("Usage: better-ctx sessions [list|show [id]|cleanup [days]]");
            std::process::exit(1);
        }
    }
}
    ...
    match action {
        ...
        _ => {
            if std::path::Path::new(action).exists() {
                ...
                eprintln!("Usage: better-ctx benchmark run [path] [--json]");
                eprintln!("       better-ctx benchmark report [path]");
                std::process::exit(1);
            }
        }
    }
...
pub fn cmd_config(args: &[String]) {
    ...
    match args[0].as_str() {
            ...
            match default.save() {
                Ok(()) => {
                    let path = config::Config::path()
                        ...
                        .unwrap_or_else(|| "~/.better-ctx/config.toml".to_string());
        ...
        "set" => {
            if args.len() < 3 {
                eprintln!("Usage: better-ctx config set <key> <value>");
                std::process::exit(1);
            }
            let mut cfg = cfg;
            let key = &args[1];
        ...
        _ => {
            eprintln!("Usage: better-ctx config [init|set <key> <value>]");
            std::process::exit(1);
        }
    }
}
...
pub fn cmd_cheatsheet() {
    println!(
        "\x1b[1;36m╔══════════════════════════════════════════════════════════════╗\x1b[0m
\x1b[1;36m║\x1b[0m  \x1b[1;37mbetter-ctx Workflow Cheat Sheet\x1b[0m                     \x1b[2mv2.9.7\x1b[0m  \x1b[1;36m║\x1b[0m
\x1b[1;36m╚══════════════════════════════════════════════════════════════╝\x1b[0m
...
\x1b[1;36m━━━ MONITORING ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━\x1b[0m
  better-ctx gain          \x1b[2m# visual savings dashboard\x1b[0m
  better-ctx gain --live   \x1b[2m# live auto-updating (Ctrl+C)\x1b[0m
  better-ctx dashboard     \x1b[2m# web dashboard with charts\x1b[0m
  better-ctx wrapped       \x1b[2m# weekly savings report\x1b[0m
  better-ctx discover      \x1b[2m# find uncompressed commands\x1b[0m
  better-ctx doctor        \x1b[2m# diagnose installation\x1b[0m
  better-ctx update        \x1b[2m# self-update to latest\x1b[0m

\x1b[2m  Full guide: https://betterctx.com/docs/workflow\x1b[0m"
    );
}
...
pub fn cmd_slow_log(args: &[String]) {
    ...
    match action {
        ...
        _ => {
            eprintln!("Usage: better-ctx slow-log [list|clear]");
            std::process::exit(1);
        }
    }
}
...
pub fn cmd_tee(args: &[String]) {
    let tee_dir = match dirs::home_dir() {
        Some(h) => h.join(".better-ctx").join("tee"),
        None => {
    ...
    match action {
        "list" | "ls" => {
            if !tee_dir.exists() {
                println!("No tee logs found (~/.better-ctx/tee/ does not exist)");
                return;
            }
            ...
            println!("\nUse 'better-ctx tee clear' to delete all logs.");
        }
        "clear" | "purge" => {
            if !tee_dir.exists() {
        ...
        "show" => {
            ...
            if filename.is_none() {
                eprintln!("Usage: better-ctx tee show <filename>");
                std::process::exit(1);
            }
        ...
        _ => {
            eprintln!("Usage: better-ctx tee [list|clear|show <file>|last]");
            std::process::exit(1);
        }
    }
}
    ...
    match action {
        "list" | "ls" => match crate::core::filters::FilterEngine::load() {
            ...
            None => {
                ...
                println!("Create one: better-ctx filter init");
            }
        },
        "validate" => {
            let path = args.get(1);
            if path.is_none() {
                eprintln!("Usage: better-ctx filter validate <file.toml>");
                std::process::exit(1);
            }
        ...
        _ => {
            eprintln!("Usage: better-ctx filter [list|validate <file>|init]");
            std::process::exit(1);
        }
    }
}
...
pub fn cmd_init(args: &[String]) {
    ...
    if !agents.is_empty() {
        ...
        println!("\nRun 'better-ctx gain' after using some commands to see your savings.");
        return;
    }
    ...
    if dry_run {
        ...
        println!("\nbetter-ctx init --dry-run\n");
        println!("  Would modify:  {rc}");
        println!("  Would backup:  {rc}.better-ctx.bak");
        ...
        println!("  Would create:  ~/.better-ctx/");
        println!("  Binary:        {binary}");
        println!("\n  Safety: aliases auto-fallback to original command if better-ctx is removed.");
    ...
    println!("\nbetter-ctx init complete (24 aliases installed)");
    println!();
    println!("  Disable temporarily:  better-ctx-off");
    println!("  Re-enable:            better-ctx-on");
    println!("  Check status:         better-ctx-status");
    println!("  Full uninstall:       better-ctx uninstall");
    println!("  Diagnose issues:      better-ctx doctor");
    println!("  Preview changes:      better-ctx init --global --dry-run");
    println!();
    if is_powershell {
...
fn backup_shell_config(path: &std::path::Path) {
    ...
    let bak = path.with_extension("better-ctx.bak");
...
pub fn init_powershell(binary: &str) {
    ...
    let functions = format!(
        r#"
# better-ctx shell hook — transparent CLI compression (90+ patterns)
if (-not $env:BETTER_CTX_ACTIVE -and -not $env:BETTER_CTX_DISABLED) {{
  $LeanCtxBin = "{binary_escaped}"
  function _lc {{
    if ($env:BETTER_CTX_DISABLED -or [Console]::IsOutputRedirected) {{ & @args; return }}
    & $LeanCtxBin -c @args
    if ($LASTEXITCODE -eq 127 -or $LASTEXITCODE -eq 126) {{
      & @args
    }}
  }}
  function better-ctx-raw {{ $env:BETTER_CTX_RAW = '1'; & @args; Remove-Item Env:BETTER_CTX_RAW -ErrorAction SilentlyContinue }}
  if (Get-Command better-ctx -ErrorAction SilentlyContinue) {{
...
"#
    ...
    if let Ok(existing) = std::fs::read_to_string(&profile_path) {
        if existing.contains("better-ctx shell hook") {
            let cleaned = remove_better_ctx_block_ps(&existing);
            match std::fs::write(&profile_path, format!("{cleaned}{functions}")) {
                Ok(()) => {
                    println!("Updated better-ctx functions in {}", profile_path.display());
                    println!("  Binary: {binary}");
                    return;
                }
                Err(e) => {
        ...
        .open(&profile_path)
    {
        Ok(mut f) => {
            ...
            println!("Added better-ctx functions to {}", profile_path.display());
            println!("  Binary: {binary}");
        }
...
fn remove_better_ctx_block_ps(content: &str) -> String {
    let mut result = String::new();
    let mut in_block = false;
    let mut brace_depth = 0i32;

    for line in content.lines() {
        if line.contains("better-ctx shell hook") {
            in_block = true;
            continue;
        }
        if in_block {
...
pub fn init_fish(binary: &str) {
    ...
    let aliases = format!(
        ...
        \n\
        function better-ctx-on\n\
        \tfor _lc_cmd in $_better_ctx_cmds\n\
        \t\talias $_lc_cmd '_lc '$_lc_cmd\n\
        \tend\n\
        \talias k '_lc kubectl'\n\
        \tset -gx BETTER_CTX_ENABLED 1\n\
        \techo 'better-ctx: ON'\n\
        end\n\
        \n\
        function better-ctx-off\n\
        \tfor _lc_cmd in $_better_ctx_cmds\n\
        \t\tfunctions --erase $_lc_cmd 2>/dev/null; true\n\
        \tend\n\
        \tfunctions --erase k 2>/dev/null; true\n\
        \tset -e BETTER_CTX_ENABLED\n\
        \techo 'better-ctx: OFF'\n\
        end\n\
        \n\
        function better-ctx-raw\n\
        \tset -lx BETTER_CTX_RAW 1\n\
        \tcommand $argv\n\
        end\n\
        \n\
        function better-ctx-status\n\
        \tif set -q BETTER_CTX_DISABLED\n\
        \t\techo 'better-ctx: DISABLED (BETTER_CTX_DISABLED is set)'\n\
        \telse if set -q BETTER_CTX_ENABLED\n\
        \t\techo 'better-ctx: ON'\n\
        \telse\n\
        \t\techo 'better-ctx: OFF'\n\
        \tend\n\
        ...
        \n\
        if not set -q BETTER_CTX_ACTIVE; and not set -q BETTER_CTX_DISABLED; and test (set -q BETTER_CTX_ENABLED; and echo $BETTER_CTX_ENABLED; or echo 1) != '0'\n\
        \tif command -q better-ctx\n\
        \t\tbetter-ctx-on\n\
        \tend\n\
        end\n\
        # better-ctx shell hook — end\n"
    );
    ...
    if let Ok(existing) = std::fs::read_to_string(&config) {
        if existing.contains("better-ctx shell hook") {
            let cleaned = remove_better_ctx_block(&existing);
            match std::fs::write(&config, format!("{cleaned}{aliases}")) {
                Ok(()) => {
                    println!("Updated better-ctx aliases in {}", config.display());
                    println!("  Binary: {binary}");
                    return;
                }
                Err(e) => {
        ...
        .open(&config)
    {
        Ok(mut f) => {
            ...
            println!("Added better-ctx aliases to {}", config.display());
            println!("  Binary: {binary}");
        }
...
better-ctx-on() {{
    for _lc_cmd in "${{_better_ctx_cmds[@]}}"; do
        # shellcheck disable=SC2139
        alias "$_lc_cmd"='_lc '"$_lc_cmd"
    done
    alias k='_lc kubectl'
    export BETTER_CTX_ENABLED=1
    echo "better-ctx: ON"
}}
...
better-ctx-off() {{
    ...
    unset BETTER_CTX_ENABLED
    echo "better-ctx: OFF"
}}

better-ctx-raw() {{
    BETTER_CTX_RAW=1 command "$@"
}}

better-ctx-status() {{
    if [ -n "${{BETTER_CTX_DISABLED:-}}" ]; then
        echo "better-ctx: DISABLED (BETTER_CTX_DISABLED is set)"
    elif [ -n "${{BETTER_CTX_ENABLED:-}}" ]; then
        echo "better-ctx: ON"
    else
        echo "better-ctx: OFF"
    fi
...
if [ -z "${{BETTER_CTX_ACTIVE:-}}" ] && [ -z "${{BETTER_CTX_DISABLED:-}}" ] && [ "${{BETTER_CTX_ENABLED:-1}}" != "0" ]; then
    command -v better-ctx >/dev/null 2>&1 && better-ctx-on
fi
# better-ctx shell hook — end
"#
    );
    ...
    if let Ok(existing) = std::fs::read_to_string(&rc_file) {
        if existing.contains("better-ctx shell hook") {
            ...
            match std::fs::write(&rc_file, format!("{cleaned}{aliases}")) {
                Ok(()) => {
                    println!("Updated better-ctx aliases in {}", rc_file.display());
                    println!("  Binary: {binary}");
                    return;
                }
                Err(e) => {
        ...
        .open(&rc_file)
    {
        Ok(mut f) => {
            use std::io::Write;
            let _ = f.write_all(aliases.as_bytes());
            println!("Added better-ctx aliases to {}", rc_file.display());
            println!("  Binary: {binary}");
        }
...
fn write_env_sh_for_containers(aliases: &str) {
    let env_sh = match dirs::home_dir() {
        Some(h) => h.join(".better-ctx").join("env.sh"),
        None => return,
    };
...
fn print_docker_env_hints(is_zsh: bool) {
    ...
    let env_sh = dirs::home_dir()
        ...
        .unwrap_or_else(|| "/root/.better-ctx/env.sh".to_string());
...
fn remove_better_ctx_block(content: &str) -> String {
    // New format uses explicit end marker; old format ends at first top-level `fi`/`end`.
    if content.contains("# better-ctx shell hook — end") {
        return remove_better_ctx_block_by_marker(content);
    }
    remove_better_ctx_block_legacy(content)
}

fn remove_better_ctx_block_by_marker(content: &str) -> String {
    let mut result = String::new();
    let mut in_block = false;

    for line in content.lines() {
        if !in_block && line.contains("better-ctx shell hook") && !line.contains("end") {
            in_block = true;
            continue;
        }
        if in_block {
            if line.trim() == "# better-ctx shell hook — end" {
                in_block = false;
            }
            continue;
        }
...
fn remove_better_ctx_block_legacy(content: &str) -> String {
    let mut result = String::new();
    let mut in_block = false;
...
pub fn cmd_theme(args: &[String]) {
    ...
    match sub {
        "list" => {
            ...
            println!();
            println!("  {d}Set theme: better-ctx theme set <name>{r}");
            println!();
        }
        "set" => {
            if args.len() < 2 {
                eprintln!("Usage: better-ctx theme set <name>");
                std::process::exit(1);
            }
            let name = &args[1];
        ...
        "import" => {
            if args.len() < 2 {
                eprintln!("Usage: better-ctx theme import <path>");
                std::process::exit(1);
            }
                ...
                Ok(content) => match toml::from_str::<theme::Theme>(&content) {
                    Ok(imported) => match theme::save_theme(&imported) {
                        Ok(()) => {
                            ...
                            let _ = cfg.save();
                            println!(
                                "  {sc}✓{r} Imported theme '{name}' → ~/.better-ctx/theme.toml",
        ...
        _ => {
            eprintln!("Usage: better-ctx theme [list|set|export|import|preview]");
            std::process::exit(1);
        }
    }
}
...
mod tests {
    ...
    #[test]
    fn test_remove_better_ctx_block_posix() {
        let input = r#"# existing config
export PATH="$HOME/bin:$PATH"

# better-ctx shell hook — transparent CLI compression (90+ patterns)
if [ -z "$BETTER_CTX_ACTIVE" ]; then
...
"#;
        ...
        assert!(!result.contains("better-ctx"), "block should be removed");
    ...
    #[test]
    fn test_remove_better_ctx_block_fish() {
        let input = "# other fish config\nset -x FOO bar\n\n# better-ctx shell hook — transparent CLI compression (90+ patterns)\nif not set -q BETTER_CTX_ACTIVE\n\talias git 'better-ctx -c git'\n\talias npm 'better-ctx -c npm'\nend\n\n# more config\nset -x BAZ qux\n";
        let result = remove_better_ctx_block(input);
        assert!(!result.contains("better-ctx"), "block should be removed");
    ...
    #[test]
    fn test_remove_better_ctx_block_ps() {
        let input = "# PowerShell profile\n$env:FOO = 'bar'\n\n# better-ctx shell hook — transparent CLI compression (90+ patterns)\nif (-not $env:BETTER_CTX_ACTIVE) {\n  $LeanCtxBin = \"C:\\\\bin\\\\better-ctx.exe\"\n  function git { & $LeanCtxBin -c \"git $($args -join ' ')\" }\n}\n\n# other stuff\n$env:EDITOR = 'vim'\n";
        let result = remove_better_ctx_block_ps(input);
        assert!(
            !result.contains("better-ctx shell hook"),
    ...
    #[test]
    fn test_remove_better_ctx_block_ps_nested() {
        let input = "# PowerShell profile\n$env:FOO = 'bar'\n\n# better-ctx shell hook — transparent CLI compression (90+ patterns)\nif (-not $env:BETTER_CTX_ACTIVE) {\n  $LeanCtxBin = \"better-ctx\"\n  function _lc {\n    & $LeanCtxBin -c \"$($args -join ' ')\"\n  }\n  if (Get-Command better-ctx -ErrorAction SilentlyContinue) {\n    function git { _lc git @args }\n    foreach ($c in @('npm','pnpm')) {\n      if ($a) {\n        Set-Variable -Name \"_lc_$c\" -Value $a.Source -Scope Script\n      }\n    }\n  }\n}\n\n# other stuff\n$env:EDITOR = 'vim'\n";
        let result = remove_better_ctx_block_ps(input);
        assert!(
            !result.contains("better-ctx shell hook"),
            "block should be removed"
    ...
    #[test]
    fn test_remove_block_no_better_ctx() {
    ...
    fn test_powershell_hook_contains_pipe_guard() {
        let hook = "function _lc { if ($env:BETTER_CTX_DISABLED -or [Console]::IsOutputRedirected) { & @args; return } }";
        assert!(
    ...
    #[test]
    fn test_remove_better_ctx_block_new_format_with_end_marker() {
        let input = r#"# existing config
export PATH="$HOME/bin:$PATH"

# better-ctx shell hook — transparent CLI compression (90+ patterns)
_better_ctx_cmds=(git npm pnpm)

better-ctx-on() {
    for _lc_cmd in "${_better_ctx_cmds[@]}"; do
        alias "$_lc_cmd"='better-ctx -c '"$_lc_cmd"
    done
    export BETTER_CTX_ENABLED=1
    echo "better-ctx: ON"
}
...
if [ -z "${BETTER_CTX_ACTIVE:-}" ] && [ "${BETTER_CTX_ENABLED:-1}" != "0" ]; then
    better-ctx-on
fi
# better-ctx shell hook — end
...
"#;
        let result = remove_better_ctx_block(input);
        assert!(!result.contains("better-ctx-on"), "block should be removed");
        assert!(!result.contains("better-ctx shell hook"), "marker removed");
        assert!(result.contains("export PATH"), "other content preserved");
        assert!(
            result.contains("export EDITOR"),
            "trailing content preserved"
