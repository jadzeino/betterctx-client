...
pub fn run() {
    ...
    if removed_any {
        println!("  ──────────────────────────────────");
        println!("  better-ctx configuration removed.\n");
    } else {
        println!("  Nothing to remove — better-ctx was not configured.\n");
    }
...
fn remove_shell_hook(home: &Path) -> bool {
    ...
    for rc in &rc_files {
        ...
        let cleaned = remove_better_ctx_block(&content);
        if cleaned.trim() != content.trim() {
            let bak = rc.with_extension("better-ctx.bak");
...
fn remove_mcp_configs(home: &Path) -> bool {
    ...
    for (name, path) in &configs {
        ...
        if let Some(cleaned) = remove_better_ctx_from_json(&content) {
    ...
    if zed_path.exists() {
        if let Ok(content) = fs::read_to_string(&zed_path) {
            if content.contains("better-ctx") {
                println!(
                    "  ⚠ Zed: manually remove better-ctx from {}",
...
fn remove_rules_files(home: &Path) -> bool {
    let rules_files: Vec<(&str, PathBuf)> = vec![
        ...
        (
            "Gemini CLI (legacy)",
            home.join(".gemini/rules/better-ctx.md"),
        ),
        ...
        ("Qwen Code", home.join(".qwen/rules/better-ctx.md")),
        ("Trae", home.join(".trae/rules/better-ctx.md")),
        (
            "Amazon Q Developer",
            home.join(".aws/amazonq/rules/better-ctx.md"),
        ),
        ("JetBrains IDEs", home.join(".jb-rules/better-ctx.md")),
        (
            "Antigravity",
            home.join(".gemini/antigravity/rules/better-ctx.md"),
        ),
...
fn remove_hook_files(home: &Path) -> bool {
    let hook_files: Vec<PathBuf> = vec![
        ...
        home.join(".gemini/hooks/better-ctx-redirect-gemini.sh"),
        home.join(".gemini/hooks/better-ctx-hook-gemini.sh"),
        home.join(".codex/hooks/better-ctx-rewrite-codex.sh"),
    ];
...
fn remove_data_dir(home: &Path) -> bool {
    let data_dir = home.join(".better-ctx");
    if !data_dir.exists() {
    ...
    match fs::remove_dir_all(&data_dir) {
        Ok(_) => {
            println!("  ✓ Data directory removed (~/.better-ctx/)");
            true
        }
        Err(e) => {
            eprintln!("  ✗ Failed to remove ~/.better-ctx/: {e}");
            false
        }
    }
}
...
fn print_binary_removal_instructions() {
    ...
    if binary_path.contains(".cargo") {
        println!("    cargo uninstall better-ctx\n");
    } else if binary_path.contains("homebrew") || binary_path.contains("Cellar") {
        println!("    brew uninstall better-ctx\n");
    } else {
...
fn remove_better_ctx_from_json(content: &str) -> Option<String> {
    ...
    if let Some(servers) = parsed.get_mut("mcpServers").and_then(|s| s.as_object_mut()) {
        modified |= servers.remove("better-ctx").is_some();
    }
