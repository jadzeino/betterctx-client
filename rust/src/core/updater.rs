...
pub fn run(args: &[String]) {
    ...
    println!();
    println!("  \x1b[1m◆ better-ctx updater\x1b[0m  \x1b[2mv{CURRENT_VERSION}\x1b[0m");
    println!("  \x1b[2mChecking github.com/jadzeino/betterctx-client …\x1b[0m");
    ...
    if check_only {
        println!("Run 'better-ctx update' to install.");
        return;
    }
    ...
    println!();
    println!("  \x1b[1;32m✓ Updated to better-ctx v{latest_tag}\x1b[0m");
...
fn refresh_shell_aliases(home: &std::path::Path) {
    ...
    for (rc_file, shell_name) in shell_configs {
        ...
        if !content.contains("better-ctx shell hook") {
            continue;
        }
    ...
    if !updated {
        println!(
            "    \x1b[2m—\x1b[0m No shell aliases to refresh (run 'better-ctx setup' to install)"
        );
    }
}
...
fn fetch_latest_release() -> Result<serde_json::Value, String> {
    let response = ureq::get(GITHUB_API_RELEASES)
        .header("User-Agent", &format!("better-ctx/{CURRENT_VERSION}"))
...
fn deferred_windows_update(
    ...
    let pending_path = target_exe.with_file_name("better-ctx-pending.exe");
    ...
    let script = format!(
        r#"@echo off
echo Waiting for better-ctx to be released...
:retry
...
if errorlevel 1 (
    ...
    echo Update failed. Please close all editors and run: better-ctx update
    pause
    exit /b 1
)
del /f "{old}" >nul 2>&1
...
"#,
    ...
    let script_path = target_exe.with_file_name("better-ctx-update.bat");
...
fn extract_from_tar_gz(data: &[u8]) -> Result<Vec<u8>, String> {
    ...
    for entry in archive.entries().map_err(|e| e.to_string())? {
        ...
        if name == "better-ctx" || name == "better-ctx.exe" {
            let mut bytes = Vec::new();
            entry.read_to_end(&mut bytes).map_err(|e| e.to_string())?;
            return Ok(bytes);
        }
    }
    Err("better-ctx binary not found inside archive".to_string())
}
...
fn extract_from_zip(data: &[u8]) -> Result<Vec<u8>, String> {
    ...
    for i in 0..zip.len() {
        ...
        if name == "better-ctx.exe" || name == "better-ctx" {
            let mut bytes = Vec::new();
            file.read_to_end(&mut bytes).map_err(|e| e.to_string())?;
            return Ok(bytes);
        }
    }
    Err("better-ctx binary not found inside zip archive".to_string())
}
...
fn platform_asset_name() -> String {
    ...
    let target = match (os, arch) {
        ...
        _ => {
            eprintln!(
                ...
                https://github.com/jadzeino/betterctx-client/releases/latest"
            );
            std::process::exit(1);
        }
    };
    ...
    if os == "windows" {
        format!("better-ctx-{target}.zip")
    } else {
        format!("better-ctx-{target}.tar.gz")
    }
}
