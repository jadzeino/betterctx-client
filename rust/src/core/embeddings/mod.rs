...
impl EmbeddingEngine {
    ...
    /// Load from default model directory (~/.better-ctx/models/).
    ...
    /// Resolve the model directory (respects BETTER_CTX_MODELS_DIR env).
    pub fn model_directory() -> PathBuf {
        if let Ok(dir) = std::env::var("BETTER_CTX_MODELS_DIR") {
            return PathBuf::from(dir);
        }
        if let Some(home) = dirs::home_dir() {
            return home.join(".better-ctx").join("models");
        }
        PathBuf::from("models")
    }
...
mod tests {
    ...
    fn model_directory_env_override_and_availability() {
        let unique = "/tmp/better_ctx_test_embed_42xyz";
        std::env::set_var("BETTER_CTX_MODELS_DIR", unique);
        let dir = EmbeddingEngine::model_directory();
        assert_eq!(dir.to_string_lossy(), unique);
        assert!(!EmbeddingEngine::is_available());
        std::env::remove_var("BETTER_CTX_MODELS_DIR");
    }
}
