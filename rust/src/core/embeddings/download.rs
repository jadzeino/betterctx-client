...
//! Files are cached in `~/.better-ctx/models/` and only downloaded once.

use std::io::Read;
use std::path::{Path, PathBuf};

const HF_BASE: &str = "https://huggingface.co/sentence-transformers/all-MiniLM-L6-v2/resolve/main";
const USER_AGENT: &str = concat!("better-ctx/", env!("CARGO_PKG_VERSION"));
...
mod tests {
    ...
    fn verify_fails_on_empty_dir() {
        let dir = std::env::temp_dir().join("better_ctx_test_verify_empty");
