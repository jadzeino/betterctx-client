...
impl Metrics {
    ...
    pub fn to_otel_attributes(&self) -> Vec<(&'static str, String)> {
        ...
        vec![
            ...
            ("better_ctx.tokens.saved", snap.tokens_saved.to_string()),
            (
                "better_ctx.tool.calls.total",
                snap.tool_calls_total.to_string(),
            ),
            (
                "better_ctx.tool.calls.error",
                snap.tool_calls_error.to_string(),
            ),
            (
                "better_ctx.tool.latency_avg_ms",
                format!("{:.2}", snap.tool_call_avg_latency_ms),
            ),
            (
                "better_ctx.search.queries",
                snap.search_queries_total.to_string(),
            ),
            (
                "better_ctx.search.latency_avg_ms",
                format!("{:.2}", snap.search_avg_latency_ms),
            ),
            (
                "better_ctx.embedding.inferences",
                snap.embedding_inferences.to_string(),
            ),
            (
                "better_ctx.embedding.latency_avg_ms",
                format!("{:.2}", snap.embedding_avg_latency_ms),
            ),
            (
                "better_ctx.cache.hit_rate",
                format!("{:.4}", snap.cache_hit_rate),
            ),
            (
                "better_ctx.compression.ratio",
                format!("{:.4}", snap.compression_ratio),
            ),
            (
                "better_ctx.session.uptime_secs",
...
mod tests {
    ...
    fn snapshot_json() {
        ...
        assert_eq!(parsed["better_ctx"]["tokens"]["saved"], 200);
    }
