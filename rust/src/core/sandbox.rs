...
fn execute_with_file(
    ...
    let tmp_dir = std::env::temp_dir().join("better-ctx-sandbox");
...
mod tests {
    ...
    fn sandbox_env_is_set() {
        let result = execute(
            "python",
            "import os; print(os.environ.get('BETTER_CTX_SANDBOX', 'missing'))",
            None,
        );
        assert_eq!(result.exit_code, 0);
