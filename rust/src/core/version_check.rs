...
const VERSION_URL: &str = "https://betterctx.com/version.txt";
...
fn cache_path() -> Option<PathBuf> {
    dirs::home_dir().map(|h| h.join(".better-ctx/latest-version.json"))
}
...
/// Spawn a background thread to fetch latest version from betterctx.com/version.txt
/// and write the result to ~/.better-ctx/latest-version.json.
...
pub fn get_update_banner() -> Option<String> {
    ...
    if is_newer(&cache.latest, CURRENT_VERSION) {
        Some(format!(
            "  \x1b[33m\x1b[1m\u{27F3} Update available: v{CURRENT_VERSION} \u{2192} v{}\x1b[0m  \x1b[2m\u{2014} run:\x1b[0m \x1b[1mbetter-ctx update\x1b[0m",
            cache.latest
        ))
    } else {
        None
