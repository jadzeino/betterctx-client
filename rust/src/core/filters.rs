...
impl FilterEngine {
    pub fn load() -> Option<Self> {
        let dir = dirs::home_dir()?.join(".better-ctx").join("filters");
        if !dir.exists() {
            return None;
        }
        ...
        for entry in entries.flatten() {
            ...
            if let Ok(content) = std::fs::read_to_string(&path) {
                match toml::from_str::<FilterFile>(&content) {
                    ...
                    Err(e) => {
                        eprintln!("better-ctx: filter parse error in {}: {e}", path.display());
                    }
                }
            }
        }
...
pub fn create_example_filter() -> Result<String, String> {
    ...
    let content = r#"# better-ctx custom filter example
# Place .toml files in ~/.better-ctx/filters/ to define custom compression rules.
