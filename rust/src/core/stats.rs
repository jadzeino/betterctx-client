...
fn load_mcp_live() -> Option<serde_json::Value> {
    let path = dirs::home_dir()?.join(".better-ctx/mcp-live.json");
...
pub fn format_cep_report() -> String {
    ...
    if cep.sessions == 0 && live.is_none() {
        return format!(
            "{d}No CEP sessions recorded yet.{r}\n\
             Use better-ctx as an MCP server in your editor to start tracking.\n\
    ...
    if avg_score >= 70.0 {
        o.push(format!(
            "    {sc}✓{r} Great score! You're using better-ctx effectively"
        ));
    }
    o.push(String::new());
...
pub fn format_gain_themed(t: &Theme) -> String {
    ...
    if store.total_commands == 0 {
        return format!(
            "{d}No commands recorded yet.{r} Use {cmd}better-ctx -c \"command\"{r} to start tracking.",
            cmd = t.secondary.fg(),
        );
    }
...
fn build_tips(store: &StatsStore) -> Vec<String> {
    ...
    if store.daily.len() >= 7 {
        tips.push("Run better-ctx gain --graph for a 30-day sparkline chart.".into());
    }
    ...
    if cfg.theme == "default" {
        tips.push(
            "Customize your dashboard! Try: better-ctx theme set cyberpunk (or neon, ocean, sunset, monochrome)".into(),
        );
        tips.push(
            "Want a unique look? Run better-ctx theme list to see all available themes.".into(),
        );
    } else {
        tips.push(format!(
            "Current theme: {}. Run better-ctx theme list to explore others.",
            cfg.theme
        ));
    }

    tips.push(
        "Create your own theme with better-ctx theme create <name> and set custom colors!".into(),
    );
...
pub fn format_gain_graph() -> String {
    ...
    if store.daily.is_empty() {
        return format!("{d}No daily data yet.{r} Use better-ctx for a few days to see the graph.");
    }
