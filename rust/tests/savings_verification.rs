use better_ctx::core::patterns::git;
...
fn verify_cep_delta_tracking_prevents_overcounting() {
    ...
    let test_dir = std::env::temp_dir().join(format!("better-ctx-cep-test-{}", std::process::id()));
    let better_ctx_dir = test_dir.join(".better-ctx");
    let _ = std::fs::create_dir_all(&better_ctx_dir);
    let stats_path = better_ctx_dir.join("stats.json");
    ...
    better_ctx::core::stats::record_cep_session(70, 5, 10, 1000, 600, &modes, 10, "Standard");

    let store1 = better_ctx::core::stats::load();
    ...
    better_ctx::core::stats::record_cep_session(75, 8, 15, 2000, 1200, &modes, 20, "Standard");

    let store2 = better_ctx::core::stats::load();
...
fn audit_full_savings_pipeline() {
    use better_ctx::core::cache::SessionCache;
    use better_ctx::core::tokens::count_tokens;
    ...
    // 1. ctx_read — first read in full mode: savings should be ~0
    {
        ...
        let output = better_ctx::tools::ctx_read::handle(
            &mut cache,
            tmp.to_str().unwrap(),
            "full",
            CrpMode::Off,
        ...
        let output2 = better_ctx::tools::ctx_read::handle(
            &mut cache,
            tmp.to_str().unwrap(),
            "full",
            CrpMode::Off,
    ...
    // 4. ctx_tree — fair comparison (same depth)
    {
        ...
        let (output, raw_tokens) = better_ctx::tools::ctx_tree::handle(dir, 2, false);
    ...
    // 5. ctx_shell — real compression
    {
        ...
        let compressed = better_ctx::core::patterns::git::compress("git log -p", &raw_output)
