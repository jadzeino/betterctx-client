...
validate_rewrite() {
    ...
    local result
    result=$(echo "$input" | BETTER_CTX_DISABLED=1 "$BIN" hook rewrite 2>/dev/null)
...
validate_rewrite "self-skip"        '{"tool_name":"Bash","command":"better-ctx read main.rs"}'                            "passthrough"
...
TMPSCRIPT="/tmp/better_ctx_prerelease_hook_$$.sh"
python3 -c "
