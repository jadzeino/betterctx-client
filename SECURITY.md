...
If you discover a security vulnerability in better-ctx, please report it privately:

- **Email**: yves@pounce.ch
- **GitHub**: [Create a private security advisory](https://github.com/jadzeino/betterctx-client/security/advisories/new)
...
## What better-ctx Does (and Doesn't Do)

better-ctx is a **local-only CLI tool and MCP server**. Understanding its scope helps assess risk:
...
- Store statistics in `~/.better-ctx/stats.json` (command counts, token savings)
- Store session state in `~/.better-ctx/sessions/` (task context, findings)
...
- **No network crates**: better-ctx has zero HTTP/networking dependencies
...
**How to verify better-ctx yourself:**
1. Build from source: `cargo install better-ctx` (compiles on your machine)
2. Compare SHA256 checksums against our [GitHub Releases](https://github.com/jadzeino/betterctx-client/releases)
...
better-ctx --version
./target/release/better-ctx --version
```

SHA256 checksums for all release binaries are published in each [GitHub Release](https://github.com/jadzeino/betterctx-client/releases).
...
- **General questions**: [GitHub Discussions](https://github.com/jadzeino/betterctx-client/discussions)
- **Discord**: [Join our server](https://discord.gg/betterctx)
